...
class UpdateViewModel @Inject constructor(
    ...
    private fun onDownloadComplete() {
        if (updateRepository.shouldRunCleanup()) {
            // Exclude the file we just downloaded — it must survive until after install.
            val oldApks = updateRepository.findDownloadedApks()
                .filter { it.canonicalPath != pendingApkFile?.canonicalPath }
            if (oldApks.isNotEmpty()) {
                _updateState.value = UpdateState.CleanupPrompt(oldApks)
                return
            }
            // No old APKs found — silently mark cleanup as done.
